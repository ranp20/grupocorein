(function($){
        ...
        let xxx =`
          <div class="single-g-item d-inline-block m-2">
            ...
            <a class="popup-link" href="${this.result}">
              <img class="admin-gallery-img" src="${this.result}" alt="No Image Found">
            </a>
          </div>
        `;
  ...
  function getCiudades(url,provincia_id){
    $.get(url+'?provincia_id='+provincia_id,function(data){
      let response = data.data;
      let view_html = ``;
      $.each(response , function(key, value){
        view_html += `<option value="${value.id}">${value.name}</option>`;
      ...
      let start = `<option value="">Select One</option>`;
      $('#ciudad_id').html(start+view_html);
    });
  };
  function getDistritos(url,ciudad_id){
    $.get(url+'?ciudad_id='+ciudad_id,function(data){
      let response = data.data;
      let view_html = ``;
      $.each(response , function(key, value){
        view_html += `<option value="${value.id}">${value.name}</option>`;
      ...
      let start = `<option value="">Select One</option>`;
      $('#distrito_id').html(start+view_html);
    });
  };
  $(document).on('change','#provincia_id',function(){
    let category_id = $(this).val();
    let url = $(this).attr('data-href');
    getCiudades(url,category_id);
  });
  $(document).on('change','#ciudad_id',function(){
    let ciudad_id = $(this).val();
    let url = $(this).attr('data-href');
